...
export function ChatBubble({ variant = "received", layout = "default", className, children }) {
  return (
    <div
      className={cn(
...
export function ChatBubbleMessage({ variant = "received", isLoading, className, children }) {
  return (
    <div
      className={cn(
        "rounded-lg p-3",
        variant === "sent" ? "bg-gradient-to-r from-indigo-600 to-blue-500 text-white" : "bg-slate-50 border border-slate-200",
        className
      )}
    >
      {isLoading ? (
